  ...
  return (
    <div className="p-3 border rounded-md">
      ...
      <div className="flex flex-col w-full gap-4">
        <Select onValueChange={(selectedValue) => setSelectedEndPoint(selectedValue)}>
          <SelectTrigger>
            <SelectValue placeholder="End Point" />
          </SelectTrigger>
          <SelectContent>
            {addresses.map((address, index) => (
              <SelectItem key={index} value={address.zip}>{address.name}</SelectItem>
            ))}
...
export default EndAddressOptions;
